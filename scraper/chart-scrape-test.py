...
if __name__ == "__main__":
    # 1. Ask user for the stock symbol dynamically
    user_input = input("Enter the Stock Symbol/Ticker (e.g., NEPSE, NICA, NABIL): ")
    
    # Clean up input (strip spaces and force uppercase to match NepseAlpha standard)
    target_symbol = user_input.strip().upper()
    
    # Fallback to NEPSE if the user just presses Enter
    if not target_symbol:
        target_symbol = "NEPSE"
        print("No symbol entered. Defaulting to 'NEPSE'.")
         
    target_timeframe = "1D"         
    start_date = "2020-01-01"       
    end_date = datetime.today().strftime('%Y-%m-%d')
    
    print(f"\nFetching {target_timeframe} chart data for {target_symbol}...")
    ...
    if df is not None:
        ...
        # 2. File name dynamically matches the symbol provided
